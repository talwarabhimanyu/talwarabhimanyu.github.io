...
Both $$u_j^{T}y$$ and $$\frac {d_j^{2}} {d_j^{2} + \lambda}$$ are scalare quantities, while $$u_j$$ is a vector. Therefore, this result basically expresses our model's predicted vector $$\hat{y}$$ as a linear combination of Left Singular Vectors of X (the $$u_j$$s). Now consider the special case when $$\lambda \space = \space 0$$ - in this case our model's prediction is simply the sum of projections of the true 'target' vector $$y$$ on the Left Singular Vectors!

$$
\hat{y} = X\hat{\beta}^{Ridge} = \sum_{j=1}^{p} u_j(u_j^{T}y)
$$ 

_Note: This result is proved in the book in three lines, however I am going to elaborate on the proof below, so that every intermediate step of the proof which is not in the book is clear to the reader._
