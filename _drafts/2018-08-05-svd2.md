...
Before I delve into the proof, let me state the magic result:
...
$$
\begin{align}
\therefore \hat{\beta}^{Ridge} &= \underbrace{(V^{T})^{-1}}_{\text{Equals V $\because V^{T}V = I$}}(D^{2} + \lambda I)^{-1}\underbrace{V^{-1}V}_{\text{Equals I}}DU^{T}y\\[2ex]
&= V(D^{2} + \lambda I)^{-1}DU^{T}y\\[2ex] 
\end{align}
$$
