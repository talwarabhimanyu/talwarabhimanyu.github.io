...
Now we are trying to minimize the sum of (1) sum of squared errors, and (2) square of length of our weight vector. **Intuitively, we are saying that we want to find a $$\beta$$ which reduces the sum of squared errors, but which is itself not too large a vector. We express our preference for how 'small' we want $$\beta$$ to be through our choice of the 'hyperparamter' $$\lambda$$.**
...
_Note: This solution in $$Eq. zz1$$ alwats exists for $\lambda > 0$$ - see Appendix for a simple proof._

## Use SVD to Gain Insights into Ridge Regression
We know now that using SVD we can factor $$X$$ as follows:

$$
X = U \space D \space V^{T} \tag{yy} 
$$

We will substitute $$X$$ with its factorization in $$Eq. zz$$.

$$
\begin{align}
X^{T}X &= (UDV^{T})^{T}(UDV)\\
&= (V\underbrace{D^{T}}_{= D \because D \text{is diagonal}}U^{T})(UDV)\\[2ex]
&= VD\underbrace{U^{T}U}_{= I \becasuse U \text{is Orthonormal}}DV\\[2ex]
&= 
\end{align}
$$
