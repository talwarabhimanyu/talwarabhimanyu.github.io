...
_P.S. The numeral r is the **Rank** of the matrix A. I will not delve into the details, but loosely here's what it is. Consider a set S of $$m \times 1$$ vectors which has the property that each column of A can be written as a linear combination of one or more vectors from S. Then the Rank is size of the smallest such set S which exists._
