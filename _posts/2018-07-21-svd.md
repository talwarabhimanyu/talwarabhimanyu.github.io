...
_Note: The 'projection' of a vector $$\hat a$$ on a$$\hat b$$, a unit vector (i.e. a vector whose length equals 1), is given by $$\lt \hat a \cdot \hat b \gt \hat b$$ where $$\lt \cdot \gt$$ denotes the dot product (and it is a scalar quantity). In our example, each row of A represent a vector of shape $$660 \times 1$$ (i.e. each row of $$A$$ represents a point in 660-dimensional space) and we want to compute its 'projection' on $$V_{1}$$, which is a unit vector of shape $$660 \times 1$$._
