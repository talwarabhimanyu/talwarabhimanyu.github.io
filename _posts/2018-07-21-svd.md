...
$$
A_{405 \times 660} = U_{405 \times 405} D_{405 \times 405} V^T_{405 \times 660}
$$

We pick the first vector $$V_{1}$$ and compute the 'projection' of each row of A on $$V_{1}$$.
...
In our example, each row of A represent a vector of shape $$1 \times 660$$ (i.e. each row of $$A$$ represents a point in 660-dimensional space) and we want to compute its 'projection' on $$V_{1}$$, which is a unit vector of shape $$660 \times 1$$. Using Pythonic notation, let $$A[0,:]$$ represent the first row of our image matrix. Then the projection of this row is given by:

$$
\text{Projection of A[0,:] on $V_1$} = \underbrace{<A[0,:], V_1>}_{\text{Length of projection on $V_1$}}\underbrace{V_1}_{\text{Unit vector}}
$$
