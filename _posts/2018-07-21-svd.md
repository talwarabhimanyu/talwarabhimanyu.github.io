...
$$
A = U_{m\timesr} D_{n\timesn} V^T_{r\timesn}
$$
...
Once we have found the kth Singular Vector of our matrix A, its corresponding Singular Value is denoted by $$\upsigma_{k}$$. The number $$\upsigma_{k}^{2}$$ equals the sum of squared projection lengths of each of the points in A (each row of A represents a point) on $$V_{k}$$. As expected, $$\upsigma_{1}$$ is the largest among all Singular Values.  
