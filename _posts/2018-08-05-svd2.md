...
Let me get started by restating the magic result provided in Section 3.4.1 of _The Elements of Statistical Learning_ (this result is proved in the book in three lines, however I am going to elaborate on the proof in the Appendix to this blog post so that every intermediate step which is not in the book is clear to the reader):
...
### Detailed proof of the 'magic result' from _The Elements of Statistical Learning_
...
$$
\begin{align}
y^{T}(X^{T}X + \lambda I)y &= y^{T}X^{T}Xy + \lambda y^{T}y
&= \underbrace{(Xy)^{T}(Xy)}_{\geqslant 0} + \underbrace{\lambda y^{T}y}_{> 0 \forall \lambda > 0, y \ne 0}
&> 0
\end{align}
$$
