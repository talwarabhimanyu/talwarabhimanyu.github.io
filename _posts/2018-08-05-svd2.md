...
$$
\begin{align}
\hat{\beta}^{Ridge} &= (VD^{2}V^{T} + \lambda\underbrace{I}_{\text{Replace I with $VV^{T}$}})^{-1}(UDV^{T})^{T}y\\[2ex]
&= (VD^{2}V^{T} + \lambdaVV^{T})^{-1}VDU^{T}y\\[2ex]
&= (V(D^{2} + \lambdaI)V^{T})^{-1}VDU^{T}y
\end{align}
$$
...
$$
\begin{align}
\therefore \hat{\beta}^{Ridge} &= (V^{T})^{-1}(D^{2} + \lambda I)^{-1}\underbrace{V^{-1}V}_{\text{Equals I}}DU^{T}y\\[2ex]
&= 
\end{align}
$$
