...
$$
\begin{align}
\hat{\beta}^{Ridge} &= (VD^{2}V^{T} + \lambda\underbrace{I}_{\text{Replace I with $VV^{T}$}})^{-1}UDV^{T}y\\[2ex]
&= (VD^{2}V^{T} + \lambdaVV^{T})^{-1}UDV^{T}y\\[2ex]
&= (V(D^{2} + \lambdaI)V^{T})^{-1}UDV^{T}y\\[2ex]
\text{Now use the fact that $(ABC)^{-1} = C^{-1}B^{-1}A^{-1}} &\\[2ex]
