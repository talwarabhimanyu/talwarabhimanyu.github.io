...
We will replace $$X$$ with its SVD factorization in $$Eq. zz$$.

$$
\begin{align}
X^{T}X &= (UDV^{T})^{T}(UDV)\\[2ex]
&= (V\underbrace{\space D^{T} \space}_{\text{Equals } D \space \because \space D \text{is diagonal}}U^{T})(UDV)\\[2ex]
&= VD\underbrace{\space U^{T}U \space}_{\text{Equals } I \because \space U \text{ is Orthonormal}}DV\\[2ex]
&= VD^{2}V 
\end{align}
$$
