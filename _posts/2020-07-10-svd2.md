...
According to $$Eq. bb$$, our model's 'predictions' $$\hat{y}$$ are the sum of projections of the 'truth' $$y$$ on each of the Left Singular Vectors (the $$u_j$$s). Now if $$\lambda > 0$$, then using $$Eq. 3$$ and $$Eq. bb$$, we have:
...
Using $$Eq. yy1$$ and $$Eq. yy2$$ in $$Eq. 2$$:
...
$$Eq. 3$$ is just an expanded form of $$Eq. GG$$ above. Hence proved.
